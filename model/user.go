...
type User struct {
	...
	VerificationCode   string                     `json:"verification_code" gorm:"-:all"`                         // transient registration verification code
	VerificationMethod string                     `json:"verification_method" gorm:"-:all"`                       // email or sms; never persisted
...
func CountUsersByPhone(phone string) (int64, error) {
	phone, valid := common.NormalizeChinaPhone(phone)
	if !valid {
...
func (user *User) prepareForInsert(tx *gorm.DB) error {
	...
	if user.Phone != nil {
		normalizedPhone, valid := common.NormalizeChinaPhone(*user.Phone)
		if !valid {
			return ErrPhoneInvalid
		}
		user.Phone = &normalizedPhone
		query := phoneQuery(tx, normalizedPhone)
		var count int64
		if err := query.Count(&count).Error; err != nil {
			return err
		}
